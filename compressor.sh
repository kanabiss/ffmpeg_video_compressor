...
if (( "$#" < 4 )); then
  ...
  echo ""
  echo "Example: $0 -i input.avi -o output.mp4 -p quality"
  exit
fi
...
ffmpeg -i $INPUT -preset $preset -tune zerolatency $params -vcodec libx264 -b:v $bitrate_to_set $OUTPUT -async 1 -vsync 1
echo "--------------------------"
